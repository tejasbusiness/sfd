...
function ReferenceImageUpload({ file, onChange }: ReferenceImageUploadProps) {
  ...
  return (
    <div>
      <label className={labelClasses}>Upload reference web page/website design as inspiration (optional)</label>
      <p className="mt-1 text-xs text-ink-soft">JPG, JPEG, PNG or WEBP — up to 0.5 MB.</p>
      ...
      {!file && (
        <Button type="button" variant="secondary" size="md" className="mt-2" onClick={() => inputRef.current?.click()}>
          Choose Image
        </Button>
      )}

      {file && previewUrl && (
        <div className="mt-2 flex items-center gap-3 rounded-lg border border-ink/10 bg-cream-dim p-3">
          <img
            src={previewUrl}
          ...
          <div className="flex shrink-0 gap-2">
            <Button type="button" variant="ghost" size="md" onClick={() => inputRef.current?.click()}>
              Replace
            </Button>
            <Button type="button" variant="ghost" size="md" onClick={handleRemove}>
              Remove
            </Button>
          </div>
        </div>
