...
### Contact page: details card sticky (2026-09-21)
On `/contact/` at desktop width (two-column layout, 61em and up) `.contact-card` is `position: sticky; top: 7rem`, so it stays below the sticky header while the form scrolls. Mobile and tablet (single column) are unchanged. The card can only travel while the form column is taller than the card; today they are about the same height (form about 617px, card about 587px), so the effect shows mainly when the form grows (for example validation messages). Nothing else needed changing: no ancestor clips overflow.
